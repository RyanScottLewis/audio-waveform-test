...
class Audio::GUI::Widget::WaveformViewer < Audio::GUI::Widget::Base
  ...
  # Samples must be an Array, containing an Array of Floats.
  def clip=(value)
    raise TypeError, 'clip must be an Audio::Clip' unless value.is_a?(Audio::Clip)
    
    @clip = value
    @colors = @clip.channels.times.collect { Gosu::Color.rgba( rand(200)+150, rand(200)+150, rand(100)+150, 255/@clip.channels ) }
